...
class ChitietDathang < ApplicationRecord
  ...
  ##################### Xử lý HUI ################
  def self.create_db(min_utility)
    unless File.open("db.txt", "a+").first.nil?
      File.delete("db.txt")
      File.new("db.txt", "w+")
    end
    File.open("db.txt", "a+") do |file|
      ChitietDathang.pluck(:dondathang_id).uniq.compact.each do |i|
        dondathang_id = ChitietDathang.where(dondathang_id: i)
        file.puts("#{dondathang_id.pluck(:chitietsanpham_id).join(" ")}:#{Dondathang.where(id: i)[0].tongtien.to_i}:#{dondathang_id.pluck(:thanhtien).map { |x| x.to_i }.join(" ")}")
      end
    end
    export_hui(min_utility)
  end

  def self.export_hui(min_utility)
    main = FHM.new
    input = "db.txt"
    main.runAlgorithm(input, min_utility)
    hui = main.hash_HUI
    # in ra file kết quả HUI
    unless File.open("result.txt", "a+").first.nil?
      File.delete("result.txt")
      File.new("result.txt", "w+")
    end
    if File.open("result.txt", "a+").first.nil?
      File.open("result.txt", "a+") do |file|
        hui.each do |i|
          file.puts("#{i[0]} #UTILiTY: #{i[1]}")
        end
      end
    end
    hui
  ...
  def self.get_hui
    arr_hui = []
    unless File.open("result.txt", "r").first.nil?
      File.open("result.txt", "r").each_line do |line|
        if !line.gsub("\n", "").empty?
          arr_hui << line.split("]")[0].gsub("[", "").split(",").map { |x| x.to_i }
        end
      end
    end
    arr_hui
